...
- 模型性能最主要由“规模”决定，规模包含三个核心因素：模型参数量（N）、数据集大小（D）和训练所用的计算量（C）。相比之下，模型的具体架构（如网络的深度与宽度）对性能的影响非常小。

- 当不受其他两个因素瓶颈限制时，模型性能与N、D、C这三个规模因素中的任何一个都呈现出平滑的幂律关系。这个趋势在超过六个数量级的变化范围内都持续有效，并且没有观察到性能增长放缓的迹象。

- 只要模型参数量（N）和数据集大小（D）协同增长，模型性能就会持续提升。但如果只增加其中一个而固定另一个，就会进入“收益递减”阶段。

- 无论模型大小如何，训练损失（loss）随训练步数下降的曲线都遵循一个可预测的幂律。这意味着，通过观察训练早期的曲线，我们就可以大致预测出模型在长时间训练后能达到的最终性能水平。

- 模型在不同数据分布上的泛化（迁移）能力，与其在原始测试集上的性能高度相关。通常，在迁移到新数据分布时，性能会有一个固定的损失，但除此之外，性能的提升趋势与在训练集上是一致的。

- 大型模型比小型模型能更有效地从数据中学习。它们可以用更少的训练步数和更少的数据样本，达到与小模型相同的性能水平。
...
这个定律发现，模型的性能和三个关键因素是直接挂钩的，即:模型规模、数据量、计算量。只要持续增加这三者的投入，模型的性能就会可预测地变好。这让我们能提前计算出，要达到某个性能水平，大概需要多大的模型、多少数据和算力，从而为巨大的训练投入提供了明确的指导和信心。
...
增大 batch size 能有效降低梯度估计中的随机噪声，使优化路径更为稳定且接近理论最优方向。下图清楚地展示了一个现象：从黑点朝向中心点（目标）移动时，较大的batch size所指示的方向更接近中心点。
