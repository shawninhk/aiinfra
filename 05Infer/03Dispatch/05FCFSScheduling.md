...
**FCFS（First-Come-First-Served，先来先服务）** 是在线推理中最常见、最容易落地的排队策略。它既不是“保守即落后”，也不是“万能钥匙”。在 **连续批处理（Continuous Batching）** 成为主流之后，FCFS 更像一台稳定的“节拍器”，帮助我们将 **稳定的解码产出（Decode）** 与 **低首 token 延迟（TTFT）的预填充（Prefill）** 协同起来。

本文结合 vLLM 的工程实现与社区讨论，系统阐述 **FCFS 的原理、优缺点、工程增强手段，以及何时应升级到更“工作负载感知”的调度策略**。
