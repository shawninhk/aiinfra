...
# Continous Batching原理

Author by: 陈宇航，程治玮

## 从训练到推理：Batching技术的演进
...
## 推理过程负载特性带来的问题：Prefill和Decode

![推理两阶段示意图](images/01ContinousBatching01.png)

而推理的过程中，由于Transformer架构下模型自回归输出的本质，整个计算过程会分成Prefill和Decode两个阶段。**在Prefill阶段，所有输入标记的计算可以并行执行，而在Decode阶段，单个请求层面无法实现并行化**。

### 阶段一：Prefill（预处理阶段）
- **计算特性**：高度并行化，能够充分利用GPU的计算核心
- **处理内容**：对所有输入token进行并行计算
- **性能瓶颈**：计算密集型任务，主要受限于GPU算力

### 阶段二：Decode（解码阶段）
- **计算特性**：串行执行，每次只能生成一个token
- **处理内容**：逐步生成输出序列
- **性能瓶颈**：内存带宽密集型，需要频繁读写庞大的KV缓存

这种根本性的差异导致了传统静态批处理（Static Batching）的失效。
...
- **Decode阶段**： 访存密集，单次计算量小，通常需要通过批处理（Batching）多个并发请求来充分利用GPU资源。


![Llama-3.1-8B on an H100吞吐量和并发关系示意图](images/01ContinousBatching03.png)

如图所示，当以并发请求数为变量时，系统吞吐量在低并发区几乎呈线性增长。这是因为在访存受限的Decode阶段，增大批处理规模能有效提升GPU利用率。然而，一旦GPU利用率饱和，系统进入计算受限状态，即使再增加并发数，整体吞吐量也将趋于平稳，达到性能上限。


![Mistral-7B在单卡A100上prefill和Decode阶段不同Batch下对吞吐影响示意图](images/01ContinousBatching06.png)
由于两阶段推理特性的不同，针对不同阶段的最佳Batch大小应该是不同的，这也意味着需要针对Prefill和Decode分别进行Batch策略。**这种阶段感知的批处理思想，正是Continuous Batching技术的核心所在**。



## 吞吐量 VS 有效吞吐量
最大化推理的吞吐确实可以用来衡量整个推理系统的服务成本。针对推理系统，只考虑吞吐是远远不够的。在大模型推理的各个下游应用中。根据用户体验的延迟要求，对用户的request需要满足一系列性能服务水平目标（SLO），最常用的SLO指标如下：

- **首Token生成时间** (TTFT - Time To First Token)：指大语言模型从接收用户输入到生成并返回第一个文本标记（Token）的全部耗时。这项指标主要反映了模型处理提示（Prefill阶段）的速度。

- **单个输出标记时间** (TPOT - Time Per Output Token)：指在首个标记生成后，模型输出每个后续标记的平均耗ت时。这项指标直接反映了模型逐字生成（Decode阶段）的推理性能。

![SLO指标示意图](images/01ContinousBatching04.png)
...
**因此推理系统中的Batching策略应该在满足这些不同的SLO延迟指标的情况下，尽可能提升系统的推理吞吐。**


## 调度器和请求队列：Batching技术和推理引擎融合的接口

### 调度器的抽象：实现批处理调度的核心
此前的分析将推理引擎作为一个黑盒模型进行了整体性考量。但是，当我们深入到批处理策略的细节，特别是权衡系统吞吐量与请求延迟时，这种宏观视角便显现出其局限性。为了精准地建模和优化请求处理流程，我们必须将系统的功能解耦，**将其显式地抽象为两大模块：负责请求接收、排序和批次构建决策的调度器，以及负责执行底层模型计算的推理引擎**。
...
在 GPU 上进行批量计算比单独处理每个请求更加高效和资源节约。使用后端请求队列才能使调度器能够选择多个请求并将它们放在同一个批次中进行处理。可以这样理解，一旦请求加入到推理引擎中，就无法再改变其顺序。因此需要在用户请求和推理引擎之间创建一个中间过程，在这个中间过程中，我们对所有的请求是可以控制的。因此，**用户不会直接将请求发送到推理引擎后端，而是发送到一个 API 服务器（LLM-Server），在API服务器层面可以对队列部分进行设计来实现一些优先级划分，更优的批处理策略等**。
...
### Orca: 从 Request-level 到 Iteration-level的调度策略
...
- **Selective Batching**：在一个Iteration中，调度器会同时处理处于Prefill和Decode阶段的多个请求，这些请求的批次大小和序列长度是动态变化的。为了应对这种复杂性，Orca提出了Selective Batching机制。该机制识别出Transformer模型中不同计算操作对批处理的适应性差异

    **对于像矩阵乘法这样易于通过填充（Padding）来高效批处理的操作，它会将所有请求合并计算。**

    **而对于像Attention这样因序列长度不同而难以高效批处理的操作，则放弃批处理，将每个请求拆分出来，串行执行。**通过这种“选择性”地应用批处理的策略，Orca在提升GPU利用率的同时，避免了在关键操作上的计算浪费
...
### Continous Batching提升计算效率的本质：对两种阶段 GPU 资源的利用方式的互补

Continous Batching混合了Prefill和Decode阶段进行批处理，其优势在于：**用一种任务填补另一种任务的资源空闲时间，在调度阶段人为混合了这两种计算阶段的流水线！**
...
### Prefill first调度策略：在Decode作为瓶颈下的调度选择
...
为了减少单个请求的等待时间，推理引擎（如 vLLM 和 TGI）会在新请求到达并能放入当前批处理时，立即调度其预填充（prefill）阶段。 并行执行新请求的预填充（prefill）与所有先前请求的单次解码（decode）步骤，但由于所有操作都在同一 GPU 操作中执行，其持续时间主要由预填充决定，因此在每个请求的解码阶段，只能在该时间内生成一个输出标记。因此，这种优先处理预填充的策略虽然最小化了首个标记的生成时间，但却中断了已运行请求的解码过程。
...
### Selective Batching存在的问题：忽略了Requet长度的影响
